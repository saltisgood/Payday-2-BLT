...
public:
	symbol(HANDLE process, DWORD64 address) : sym((sym_type *)::operator new(sizeof(sym_type) + max_name_len)) {
...
DWORD DumpStackTrace(EXCEPTION_POINTERS *ep)
{
	...
	do {
		if (frame.AddrPC.Offset != 0) {
			...
			if (fnName == "RaiseException") {
				...
				PD2HOOK_LOG_ERROR("Your program has crashed");
	...
	// Display the string:
	PD2HOOK_LOG_ERROR("Uh-oh, program has crashed: " << builder.str());
	return EXCEPTION_CONTINUE_SEARCH;
}
}